...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        ...
        <form action="{{ route('workflow.summary-report.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Summary Report (PDF) <span class="text-red-500">*</span></label>
                <input type="file" name="summary_report" accept=".pdf" required
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-300">
                <p class="text-xs text-gray-400 mt-1">PDF format only, max 20MB</p>
                @error('summary_report')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-start gap-2">
                <input type="checkbox" name="verify" id="verify" value="1" required class="mt-1">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="px-6 py-2 text-white rounded-lg font-semibold hover:opacity-90" style="background:#e89600;">Submit for Review</button>
            </div>
        </form>
    </div>
</div>
